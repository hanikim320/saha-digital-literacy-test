(function(){
  ...
  const KEY="media-integrated-v4-1"; let answers={};
  ...
  function el(tag, attrs={}, children=[]){
    ...
    for(const[k,v] of Object.entries(attrs)){
      ...
      else if(k==="for"){ try{ x.htmlFor=v; }catch(e){ x.setAttribute("for", v);} }  // patch: label for
  ...
  function mountQuiz(){
    const m=document.getElementById("qMount");
    m.innerHTML="";
  ...
  function mountPref(){
    const m=document.getElementById("prefMount");
    m.innerHTML="";
    try{
      const grid=el("div",{class:"grid"}); m.appendChild(grid);
      PREFS.forEach(p=>{
        ...
        [["A",p.a,"a"],["B",p.b,"b"]].forEach(([labTxt,txt,val])=>{
          const lab=el("label",{class:"opt"+(answers[p.id]===val?" active":""),for:`p${p.id}_${val}`});
          const input=el("input",{type:"radio",name:`p${p.id}`,id:`p${p.id}_${val}`,value:val});
      ...
      m.appendChild(el("p",{class:"muted"}, "성향 문항을 불러오는 중 오류가 발생했습니다. 페이지를 새로고침해 주세요."));
      console.error(e);
    }
  }
  ...
  function renderSummary(submit=false){
    ...
    const ratio=Math.round(correct/totalQ*100);
    ...
    let aCnt=0,bCnt=0;
    PREFS.forEach(p=>{ if(answers[p.id]==="a") aCnt++; else if(answers[p.id]==="b") bCnt++; });
  ...
  function mountExplain(submit=false){
    const m=document.getElementById("explainMount");
    m.innerHTML=""; if(!submit) return;
  ...
  function mountWrongNote(submit=false){
    const m=document.getElementById("wrongNoteMount");
    m.innerHTML=""; if(!submit) return;
