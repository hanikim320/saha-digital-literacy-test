(function(){
  ...
  // 저장 키
  const KEY = "fake-news-quiz-answers-v2";
  let answers = {};
  ...
  function levelFromScore(ratio){
    // 디지털 리터러시 역량 레벨 (중1 기준)
    // 0-39%: 기초 형성 단계, 40-69%: 기본 수준, 70-89%: 표준 수준, 90-100%: 우수 수준
  ...
  function renderSummary(isSubmit=false){
    ...
    const badge = el("span", {class:"badge", style:`border-color:${lvl.color}; color:${lvl.color}`}, `점수 ${correct}/${total} (${ratio}%) · ${lvl.name}`);
    const tip = el("div", {class:"muted mt-12"}, `학습 조언: ${lvl.tip}`);

    // 간단한 능력 요약표
    const tbl = el("div", {class:"chart mt-12"});
    const items = [
      {k:"출처 확인", v: ratio>=70? "좋음":"보통"},
      {k:"과장표현 분별", v: ratio>=70? "좋음":"보통"},
      {k:"전문가/기관 인용 확인", v: ratio>=70? "좋음":"보통"},
      {k:"광고성 링크 주의", v: ratio>=70? "좋음":"보통"},
    ];
    items.forEach(it=>{
      const row = el("div", {class:"bar"}, [
        el("div", {class:"barlabel"}, it.k),
  ...
  function mountExplain(isSubmit=false){
    ...
    QUIZ.forEach(q=>{
      ...
      const body = el("div", {class:"muted"}, q.explain);
  ...
  function mountWrongNote(isSubmit=false){
    ...
    const card = el("article", {class:"card"});
    const head = el("div", {class:"h5"}, "오답노트");
    const help = el("div", {class:"muted mt-8"}, "아래 칸에 ‘왜 헷갈렸는지’와 ‘다음에 어떻게 확인할지’를 적어 보세요.");
    const ta = el("textarea", {class:"ta", placeholder:"예) 4번: 숫자가 커서 진짜 같았음 → 다음에는 기상청/정부 발표 확인하기"});
    card.appendChild(head);
    if (wrongs.length === 0){
      card.appendChild(el("div", {class:"muted mt-8"}, "모든 문항 정답! 훌륭해요. 친구에게 팁을 공유해 주세요."));
    } else {
      const list = el("ul", {});
      wrongs.forEach(w=>{
        const li = el("li", {}, `문항 ${w.id}: 정답은 ${w.answer+1}번 · 해설: ${w.explain}`);
        list.appendChild(li);
      });
  ...
  document.getElementById("submitBtn").addEventListener("click", ()=>{
    ...
    if (first){
      ...
      setTimeout(()=> document.getElementById("submitHint").textContent="", 4000);
      return;
    }
